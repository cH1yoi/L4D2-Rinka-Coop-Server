/*===========================================================================================================
	...
	本插件由「仟姬物语」修改及补充
 	QQ：892510007
 	哔哩哔哩：https://space.bilibili.com/10684945
...
#define SOUND_READY "ui/beep07.wav"
#define SOUND_NEXT "buttons/bell1.wav"
...
Handle god;
Handle safe_zone_waiting = INVALID_HANDLE;
...
ConVar Cvar_DoorLock_RpidTime;
ConVar Cvar_DoorLock_DefTime;
...
public void OnPluginStart()
{
	...
	Cvar_DoorLock_AllowLock = CreateConVar("l4d2_doorlock_plugin_enable", "1", "0=关闭插件, 1=开启插件", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	Cvar_DoorLock_GameModes = CreateConVar("l4d2_doorlock_game_mode", "versus,coop", "在这些模式中启用插件,逗号隔开不需要空格(全空=全模式启用插件)", FCVAR_NOTIFY);
	Cvar_DoorLock_ModesType = CreateConVar("l4d2_doorlock_first_scenario_mode", "1", "设置开局章节模式(0=冻结幸存者, 1=传送幸存者)", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	Cvar_DoorLock_AddCheats = CreateConVar("l4d2_doorlock_add_cheats", "3", "在安全区域锁定时设置作弊(0=无作弊, 1=无伤害, 2=无限弹药, 3=两者)", FCVAR_NOTIFY, true, 0.0, true, 3.0);
	Cvar_DoorLock_Countdown = CreateConVar("l4d2_doorlock_countdown", "5.0", "等待所有玩家加载好或者放弃等待后,多少秒解锁安全区(秒)", FCVAR_NOTIFY);
	Cvar_DoorLock_LoaderMax = CreateConVar("l4d2_doorlock_loaders_time", "10", "地图启动后,等待加载的玩家多少秒放弃等待", FCVAR_NOTIFY);
	Cvar_DoorLock_AllowGlow = CreateConVar("l4d2_doorlock_glow_enable", "1", "是否开启安全门轮廓颜色", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	Cvar_DoorLock_GlowRange = CreateConVar("l4d2_doorlock_glow_range", "500", "安全门轮廓颜色范围", FCVAR_NOTIFY);
	Cvar_DoorLock_LockColor = CreateConVar("l4d2_doorlock_lock_glow_color",	"255 0 0", "设置安全门锁定时轮廓发光颜色,用空格分隔(0-255)", FCVAR_NOTIFY);
	Cvar_DoorLock_OpenColor = CreateConVar("l4d2_doorlock_unlock_glow_color", "0 255 0", "设置安全门解锁时轮廓发光颜色,用空格分隔(0-255)", FCVAR_NOTIFY);
	Cvar_DoorLock_EnableRdy = CreateConVar("l4d2_doorlock_enable_ready_mode", "1", "启用就绪和未就绪功能", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	Cvar_DoorLock_UnrdyTime = CreateConVar("l4d2_doorlock_unready_counts", "1", "设定每回合允许玩家使用指令未准备状态的次数", FCVAR_NOTIFY);
	Cvar_DoorLock_RdyTimeUp = CreateConVar("l4d2_doorlock_readyup_time", "15.0", "放弃未准备好的队伍之前等待多长时间(秒)", FCVAR_NOTIFY);
	Cvar_DoorLock_RdyPercnt = CreateConVar("l4d2_doorlock_readyup_percent", "65.0", "准备好的玩家达到了多少就直接倒计时解锁安全区(百分比)", FCVAR_NOTIFY);
	Cvar_DoorLock_Announces = CreateConVar("l4d2_doorlock_readyup_notify", "1", "当团队准备好时显示状态面板", FCVAR_NOTIFY, true, 0.0, true, 3.0);
	Cvar_DoorLock_LeaverMsg = CreateConVar("l4d2_doorlock_leavers_notify", "0", "传送时向离开安全区玩家显示聊天文本(0=禁用, 1=聊天, 2=中心文本)", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	Cvar_DoorLock_EnableCmd = CreateConVar("l4d2_doorlock_add_commands", "!map,!buy,!shop", "添加所需的命令以防止干扰ReadyUp面板(无空格)", FCVAR_NOTIFY);
	Cvar_DoorLock_RpidTime  = CreateConVar("rime_for_rapid_treatment", "1.0", "快速打包：开门前打包所需时间", FCVAR_NOTIFY);
	Cvar_DoorLock_DefTime   = CreateConVar("default_treatment_time", "5.0", "默认打包：开门后打包所需时间", FCVAR_NOTIFY);
	...
	RegAdminCmd("sm_lock", Command_Lock, ADMFLAG_GENERIC, "强制锁定安全门");
	RegAdminCmd("sm_unck", Command_Unlock, ADMFLAG_GENERIC, "强制解锁安全门");
	RegConsoleCmd("sm_ok", Command_Ready, "将玩家的状态设置为就绪");
	RegConsoleCmd("sm_no", Command_Unready, "将玩家的状态设置为未就绪");
...
void Event_RoundFreezeEnd(Event event, const char[] name, bool dontBroadcast)
{
	...
	although_not_100 = true;
	
	g_iLoadersTime = Cvar_DoorLock_LoaderMax.IntValue;		//等待连接时长
	g_iGiveUpsTime = Cvar_DoorLock_RdyTimeUp.IntValue;		//未就绪倒计时长
	g_iUnlocksTime = Cvar_DoorLock_Countdown.IntValue;		//已就绪倒计时长
...
void Event_OnRoundEnd(Event event, const char[] name, bool dontBroadcast)
{
	...
	TerminateResurrectionTiming();
}

void TerminateResurrectionTiming()				//终止复活计时器
{
	if(safe_zone_waiting != INVALID_HANDLE)
	{
		KillTimer(safe_zone_waiting);
		safe_zone_waiting = INVALID_HANDLE;
	}
}
...
Action Timer_StartCountdownToUnlock(Handle timer)	//已就绪倒计时
{
	...
	CreateTimer(0.1, StartCountdownToUnlock, _, TIMER_FLAG_NO_MAPCHANGE);
	return Plugin_Continue;
}

Action StartCountdownToUnlock(Handle timer)		//已就绪调用的函数
{
	if(!although_not_100)
	{
		delete g_hTimer_CountdownTime;
...
Action Timer_WaitingForUnreadyPlayers(Handle timer)		//未就绪倒计时
{
	if(g_iGiveUpsTime <= 0 || g_bLeftSafeAreas)
	{
		...
		g_hTimer_CountdownTime = CreateTimer(1.0, StartCountdownToUnlock, _, TIMER_REPEAT|TIMER_FLAG_NO_MAPCHANGE);
		return Plugin_Stop;
	}
...
void Create_ShowTeamsReadyStatus(int client)
{
	...
	CreateTimer(0.5, Timer_ActivatePanel, GetClientUserId(client), TIMER_FLAG_NO_MAPCHANGE);	//面板刷新频率
}
...
public Action CheckResurrection(Handle timer)
{
	for(int i = 1; i <= MaxClients; i++)
	{
		if(!IsClientInGame(i) || GetClientTeam(i) !=2 || IsPlayerAlive(i))
			continue;
		SDKCall(EndPrepSDKCall(), i);
	}
	return Plugin_Continue;
}
...
public void Event_RoundStartinfo(Event event, const char[] name, bool dontBroadcast)
{
	...
	{
		ImmuneInjury(true);
		//PrintToChatAll("<Event_RoundStartinfo>免伤开启");
		if(safe_zone_waiting == INVALID_HANDLE)
			safe_zone_waiting = CreateTimer(1.0, CheckResurrection, _, TIMER_REPEAT|TIMER_FLAG_NO_MAPCHANGE);	//执行复活循环计时器
	}
	
	SetConVarFloat(FindConVar("first_aid_kit_use_duration"), Cvar_DoorLock_RpidTime.FloatValue);
	
	if(Cvar_DoorLock_EnableRdy.BoolValue)
		CPrintToChatAll("%t", "DoorLock RdyPercnt", Cvar_DoorLock_RdyPercnt.IntValue);
}
...
void UnTriggerSafeAreaLocksFeatures()
{
	...
	ImmuneInjury(false);
	//PrintToChatAll("<UnTriggerSafeAreaLocksFeatures>免伤关闭");
	SetConVarFloat(FindConVar("first_aid_kit_use_duration"), Cvar_DoorLock_DefTime.FloatValue);
