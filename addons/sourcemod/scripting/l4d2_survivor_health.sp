...
int iKillDefault[array] = {2, 1, 3, 1, 2, 3, 10, 20};
//这里设置爆头奖励的血量(根据g_sZombieName数组顺序设置).
int iHeadDefault[array] = {3, 2, 5, 2, 4, 5, 20, 20};
...
ConVar g_hKill[array], g_hHead[array], g_hRewardPrompt, g_hOneshotWitch, g_hLimitHealth, g_hReviveSuccess, g_hSurvivorRescued, g_hHealSuccess, g_hDefibrillator;
...
public void OnPluginStart()
{
	...
	g_hRewardPrompt = CreateConVar("health_reward_prompt",		"3",	"击杀奖励提示设置(启用多种提示就把数字相加) 0=禁用, 1=特感奖励提示(只有自己可见), 2=女巫坦克奖励提示(所有人可见).", FCVAR_NOTIFY);
	g_hOneshotWitch = CreateConVar("health_oneshot_witch",		"10",	"秒杀女巫的幸存者奖励多少血. 0=禁用.", FCVAR_NOTIFY);
	g_hLimitHealth = CreateConVar("survivor_health_Limit",		"50",	"设置幸存者获得血量奖励的最高上限.", FCVAR_NOTIFY);

	g_hReviveSuccess = CreateConVar("health_reviveSuccess",		"5",	"救起倒地的幸存者奖励多少血. 0=禁用.", FCVAR_NOTIFY);
	g_hSurvivorRescued = CreateConVar("health_survivorRescued",	"10",	"营救队友的幸存者奖励多少血. 0=禁用.", FCVAR_NOTIFY);
	g_hHealSuccess = CreateConVar("health_healSuccess",			"20",	"治愈队友的幸存者奖励多少血. 0=禁用.", FCVAR_NOTIFY);
	g_hDefibrillator = CreateConVar("health_defibrillator",		"20",	"电击器复活队友的幸存者奖励多少血. 0=禁用.", FCVAR_NOTIFY);
...
public void Event_DefibrillatorUsed(Event event, const char[] name, bool dontBroadcast)
{
	...
	if(client > 0)
	{
		if(!IsPlayerAlive(client) || !IsPlayerState(client))
		return;
	}

	if(client > 0 && IsValidClient(client) && GetClientTeam(client) == 2)
		if(subject > 0 && IsValidClient(subject) && GetClientTeam(subject) == 2)
			if(client != subject)
...
public void Event_SurvivorRescued(Event event, const char[] name, bool dontBroadcast)
{
	...
	if(rescuer > 0)
	{
		if(!IsPlayerAlive(rescuer) || !IsPlayerState(rescuer))
		return;
	}

	if(client > 0 && IsValidClient(client) && GetClientTeam(client) == 2)
		if(rescuer > 0 && IsValidClient(rescuer) && GetClientTeam(rescuer) == 2)
			if(client != rescuer)
...
public void Event_WitchHarasserSet(Event event, const char[] name, bool dontBroadcast)
{
	...
	if(client > 0 && IsValidClient(client) && GetClientTeam(client) == 2)
		PrintToChatAll("\x04[提示]\x03%s\x05嫖\x03%s\x05没给钱,正在被\x03%s\x05追杀中.", GetTrueName(client), GetWitchName(witchid), GetWitchName(witchid));//聊天窗提示.
}
...
public void Event_PlayerDeath(Event event, const char[] name, bool dontBroadcast)
{
	...
	if(attacker > 0 && IsValidClient(attacker) && GetClientTeam(attacker) == 2)
	{
		char classname[32];
...
void IsPrintToChat(int attacker, int iTotalHealth, int iReward, int iMaxHealth, char[] sType, char[] sName, bool bPlayerState)
{
	if(attacker > 0)
	{
		if(!IsClientInGame(attacker) || !IsPlayerAlive(attacker) || !IsPlayerState(attacker))
		return;
	}

	if (GetConVarInt(g_hRewardPrompt) == 1 || GetConVarInt(g_hRewardPrompt) == 3)
	{
		if (bPlayerState)
		{
			if (iTotalHealth < iMaxHealth)
				PrintToChat(attacker, "\x04[提示]\x05你%s了\x03%s\x04,\x05奖励\x03%d\x05点血量.", sType, sName, iReward);//聊天窗提示.
			else
				PrintToChat(attacker, "\x04[提示]\x05你%s了\x03%s\x04,\x05血量已达\x03%d\x05上限.", sType, sName, iMaxHealth);//聊天窗提示.
		}
	}
}
...
void IsPrintToChatAll(int attacker, int iTotalHealth, int iReward, int iMaxHealth, char[] sType, char[] sName, bool bPlayerState)
{
	...
	if (GetConVarInt(g_hRewardPrompt) == 2 || GetConVarInt(g_hRewardPrompt) == 3)
	{
		if (bPlayerState)
		{
			if (iTotalHealth < iMaxHealth)
				PrintToChatAll("\x04[提示]\x03%s\x05%s了\x03%s\x04,\x05奖励\x03%d\x05点血量.", GetTrueName(attacker), sType, sName, iReward);//聊天窗提示.
			else
...
char[] GetTrueName(int client)
{
	...
	if(Bot != 0)
		FormatEx(sName, sizeof(sName), "%N", Bot);
	else
