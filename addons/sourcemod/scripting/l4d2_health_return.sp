...
public void OnPluginStart()
{
	...
	hCountKIHLimit			= CreateConVar("l4d2_health_Limit", "100", "设置幸存者获得血量奖励的最高上限(最小值100).", FCVAR_NOTIFY);
			...
			case 1:
			{
				if (!l4d2_HLReturnset)
				{
					...
					else
					{
						...
						l4d2_HLReturnset = true;
						hCountKIH_Switch_true = false;
						PrintToChatAll("\x04[提示]\x03已开启\x05击杀特感坦克女巫血量奖励,血量上限\x04%d\x03.", CountKIHLimit);
					}
				}
				else
				{
...
public void HealSuccess(Event event, const char[] name, bool dontBroadcast)
{
	...
	if (CountKIHenabled == 1 && l4d2_HLReturnset)
	{
		...
		if (client == subject)
		{
			//PrintToChatAll("\x05[提示]\x04%s\x03治愈了自己.", clientName);//聊天窗提示.
			return;
		}
		if (GetClientTeam(client) == 2)
		{
			if (HLReturnset)
			{
				if (IsPlayerAlive(subject) && GetEntProp(subject, Prop_Send, "m_isIncapacitated") == 0)
				{
					...
					{
						PrintToChatAll("\x04[提示]\x03%s\x05治愈了\x03%s\x04,\x05血量已达\x03%d\x05上限.", clientName, subjectname, CountKIHLimit);//聊天窗提示.
					}
				}
			}
			else
...
public void KIHEvent_KillInfected(Event event, const char[] name, bool dontBroadcast)
{
	...
	int HLZClass, iClipCountHead, iClipCountShot;
	...
	if (CountKIHenabled == 1 && l4d2_HLReturnset)
	{
		if(IsValidClient(attacker) && GetClientTeam(attacker) == 2 && IsValidClient(client) && GetClientTeam(client) == 3)
		{
			...
			//PrintToChatAll("\x04[提示]\x05测试提示: %d.", PrimType);
			if (weapon != -1 && PrimType != -1)
			{
				switch (headshot)
				{
					case 0:
						...
						else
						{
							if (IsPlayerAlive(attacker))
							{
								if (GetEntProp(attacker, Prop_Send, "m_isIncapacitated") == 0)
								{
									...
									if (Attackerhealth + tmphealth + CountKIHNumHead > CountKIHLimit)
									{
										float overhealth,fakehealth;
										overhealth = float(Attackerhealth + tmphealth + CountKIHNumHead - CountKIHLimit);
									...
									if ((Attackerhealth + CountKIHNumHead) < CountKIHLimit)
										SetEntProp(attacker, Prop_Send, "m_iHealth", Attackerhealth + CountKIHNumHead, 1);
									else
									...
									if (Attackerhealth2 < CountKIHLimit)
										...
										GetAttackerNameLimit(attacker, PrimType, HLZClass, iClipCountHead);
								}
								else
								{
									if (CountKIHNumoff == 0)
						...
						else
						{
							if (IsPlayerAlive(attacker))
							{
								if (GetEntProp(attacker, Prop_Send, "m_isIncapacitated") == 0)
								{
									...
									if (Attackerhealth + tmphealth + CountKIHNumShot > CountKIHLimit)
									{
										float overhealth,fakehealth;
										overhealth = float(Attackerhealth + tmphealth + CountKIHNumShot - CountKIHLimit);
									...
									if ((Attackerhealth + CountKIHNumShot) < CountKIHLimit)
										SetEntProp(attacker, Prop_Send, "m_iHealth", Attackerhealth + CountKIHNumShot, 1);
									else
										SetEntProp(attacker, Prop_Send, "m_iHealth", CountKIHLimit, 1);
									...
									if (Attackerhealth2 < CountKIHLimit)
										...
										GetAttackerShotNameLimit(attacker, PrimType, HLZClass, iClipCountShot);
								}
								else
								{
									if (CountKIHNumoff == 0)
...
void GetAttackerNameLimit(int client, int PrimType, int HLZClass, int iClipCountHead)
{
	if (PrimType == 1 || PrimType == 2 || PrimType == 3 || PrimType == 5 || PrimType == 7 || PrimType == 8 || PrimType == 9 || PrimType == 10 || PrimType == 17 || PrimType == 19)
	{
		switch (HLZClass)
		{
			case 1: //smoker
				PrintToChat(client, "\x04[提示]\x05击杀感染者\x03舌头%s\x04,\x05奖励\x03%d\x05前置弹药,血量已达\x03%d\x05上限.", slName1, iClipCountHead, CountKIHLimit);//聊天窗提示.
			case 2: //boomer
				PrintToChat(client, "\x04[提示]\x05击杀感染者\x03胖子%s\x04,\x05奖励\x03%d\x05前置弹药,血量已达\x03%d\x05上限.", slName2, iClipCountHead, CountKIHLimit);//聊天窗提示.
			case 3: //hunter
				PrintToChat(client, "\x04[提示]\x05击杀感染者\x03猎人%s\x04,\x05奖励\x03%d\x05前置弹药,血量已达\x03%d\x05上限.", slName3, iClipCountHead, CountKIHLimit);//聊天窗提示.
			case 4: //spitter
				PrintToChat(client, "\x04[提示]\x05击杀感染者\x03口水%s\x04,\x05奖励\x03%d\x05前置弹药,血量已达\x03%d\x05上限.", slName4, iClipCountHead, CountKIHLimit);//聊天窗提示.
			case 5: //jockey
				PrintToChat(client, "\x04[提示]\x05击杀感染者\x03猴子%s\x04,\x05奖励\x03%d\x05前置弹药,血量已达\x03%d\x05上限.", slName5, iClipCountHead, CountKIHLimit);//聊天窗提示.
			case 6: //charger
				PrintToChat(client, "\x04[提示]\x05击杀感染者\x03牛牛%s\x04,\x05奖励\x03%d\x05前置弹药,血量已达\x03%d\x05上限.", slName6, iClipCountHead, CountKIHLimit);//聊天窗提示.
		}
	}
	else
	{
		...
		{
			case 1: //smoker
				PrintToChat(client, "\x04[提示]\x05击杀感染者\x03舌头%s\x04,血量已达\x03%d\x05上限.", slName1, CountKIHLimit);//聊天窗提示.
			case 2: //boomer
				PrintToChat(client, "\x04[提示]\x05击杀感染者\x03胖子%s\x04,血量已达\x03%d\x05上限.", slName2, CountKIHLimit);//聊天窗提示.
			case 3: //hunter
				PrintToChat(client, "\x04[提示]\x05击杀感染者\x03猎人%s\x04,血量已达\x03%d\x05上限.", slName3, CountKIHLimit);//聊天窗提示.
			case 4: //spitter
				PrintToChat(client, "\x04[提示]\x05击杀感染者\x03口水%s\x04,血量已达\x03%d\x05上限.", slName4, CountKIHLimit);//聊天窗提示.
			case 5: //jockey
				PrintToChat(client, "\x04[提示]\x05击杀感染者\x03猴子%s\x04,血量已达\x03%d\x05上限.", slName5, CountKIHLimit);//聊天窗提示.
			case 6: //charger
				PrintToChat(client, "\x04[提示]\x05击杀感染者\x03牛牛%s\x04,血量已达\x03%d\x05上限.", slName6, CountKIHLimit);//聊天窗提示.
		}
	}
}
...
void GetAttackerShotNameLimit(int client, int PrimType, int HLZClass, int iClipCountShot)
{
	if (PrimType == 1 || PrimType == 2 || PrimType == 3 || PrimType == 5 || PrimType == 7 || PrimType == 8 || PrimType == 9 || PrimType == 10 || PrimType == 17 || PrimType == 19)
	{
		switch (HLZClass)
		{
			case 1: //smoker
				PrintToChat(client, "\x04[提示]\x05爆头击杀感染者\x03舌头%s\x04,\x05奖励\x03%d\x05前置弹药,血量已达\x03%d\x05上限.", slName1, iClipCountShot, CountKIHLimit);//聊天窗提示.
			case 2: //boomer
				PrintToChat(client, "\x04[提示]\x05爆头击杀感染者\x03胖子%s\x04,\x05奖励\x03%d\x05前置弹药,血量已达\x03%d\x05上限.", slName2, iClipCountShot, CountKIHLimit);//聊天窗提示.
			case 3: //hunter
				PrintToChat(client, "\x04[提示]\x05爆头击杀感染者\x03猎人%s\x04,\x05奖励\x03%d\x05前置弹药,血量已达\x03%d\x05上限.", slName3, iClipCountShot, CountKIHLimit);//聊天窗提示.
			case 4: //spitter
				PrintToChat(client, "\x04[提示]\x05爆头击杀感染者\x03口水%s\x04,\x05奖励\x03%d\x05前置弹药,血量已达\x03%d\x05上限.", slName4, iClipCountShot, CountKIHLimit);//聊天窗提示.
			case 5: //jockey
				PrintToChat(client, "\x04[提示]\x05爆头击杀感染者\x03猴子%s\x04,\x05奖励\x03%d\x05前置弹药,血量已达\x03%d\x05上限.", slName5, iClipCountShot, CountKIHLimit);//聊天窗提示.
			case 6: //charger
				PrintToChat(client, "\x04[提示]\x05爆头击杀感染者\x03牛牛%s\x04,\x05奖励\x03%d\x05前置弹药,血量已达\x03%d\x05上限.", slName6, iClipCountShot, CountKIHLimit);//聊天窗提示.
		}
	}
	else
	{
		...
		{
			case 1: //smoker
				PrintToChat(client, "\x04[提示]\x05爆头击杀感染者\x03舌头%s\x04,\x05血量已达\x03%d\x05上限.", slName1, CountKIHLimit);//聊天窗提示.
			case 2: //boomer
				PrintToChat(client, "\x04[提示]\x05爆头击杀感染者\x03胖子%s\x04,\x05血量已达\x03%d\x05上限.", slName2, CountKIHLimit);//聊天窗提示.
			case 3: //hunter
				PrintToChat(client, "\x04[提示]\x05爆头击杀感染者\x03猎人%s\x04,\x05血量已达\x03%d\x05上限.", slName3, CountKIHLimit);//聊天窗提示.
			case 4: //spitter
				PrintToChat(client, "\x04[提示]\x05爆头击杀感染者\x03口水%s\x04,\x05血量已达\x03%d\x05上限.", slName4, CountKIHLimit);//聊天窗提示.
			case 5: //jockey
				PrintToChat(client, "\x04[提示]\x05爆头击杀感染者\x03猴子%s\x04,\x05血量已达\x03%d\x05上限.", slName5, CountKIHLimit);//聊天窗提示.
			case 6: //charger
				PrintToChat(client, "\x04[提示]\x05爆头击杀感染者\x03牛牛%s\x04,\x05血量已达\x03%d\x05上限.", slName6, CountKIHLimit);//聊天窗提示.
		}
	}
}
				...
				switch (Crownd)
				{
					...
					case 1:
					{
						...
						if (HLReturnset)
						{
							...
							if (Attackerhealth + tmphealth + CountKIHWitchShot > CountKIHLimit)
							{
								float overhealth,fakehealth;
								overhealth = float(Attackerhealth + tmphealth + CountKIHWitchShot - CountKIHLimit);
								if (tmphealth < overhealth)
								{
									fakehealth = 0.0;
								}
							...
							if ((Attackerhealth + CountKIHWitchShot) < CountKIHLimit)
							{
								SetEntProp(client, Prop_Send, "m_iHealth", Attackerhealth + CountKIHWitchShot, 1);
							}
							else
							{
								SetEntProp(client, Prop_Send, "m_iHealth", CountKIHLimit, 1);
