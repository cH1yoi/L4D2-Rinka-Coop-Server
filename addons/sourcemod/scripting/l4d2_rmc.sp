...
public void OnTakeOver(Event event, const char[] name, bool dontBroadcast) {
	...
	if(!id) {
		...
		PrintToChat(client, "\x05[花] \x04无法验证steamid,默认死亡状态");
		return;
	}
	for(int j = 0; j < sizeof(plList) && plList[j][i_Id]; j++) {  // 注意round_end
		if(plList[j][i_Id] == id) {
			...
			if(flag) PrintToChatAll("\x05[花] \x03%N \x04重复接管Bot, %s%s%s", client, flag & 1 ? "恢复血量" : "", flag == 3 ? "及" : (flag & 2 ? "恢复" : ""), flag & 2 ? "倒地次数" : "");
			return;
		}
	}
}
...
public Action JoinTeam(Handle timer, any uid) {
	int client = GetClientOfUserId(uid);
	if(client && isPlayer(client)) {
		PrintToChat(client, "\x04[花] \x01多人插件:\x05 %s", Enable ? "开启" : "关闭");
		PrintToChat(client, "\x05[花] \x03!setmax <人数> \x04修改人数上限, \x03!zs \x04自杀");
		PrintToChat(client, "\x05[花] \x03!jg \x04加入生还者, \x03!away \x04加入观察者");
		if(Enable) {
			PrintToChat(client, "\x04[花] \x01多倍药物:\x05 %s, \x01Tank多倍血量:\x05 %s", cMultMed.IntValue ? "已开启" : "已关闭", GetConVarInt(cMultHp) ? "已开启" : "已关闭");
...
public Action Cmd_SetMax(int client, int args) {
	if(args == 1) {
		...
		if(!client || isAdmin(client)) {  // console
			...
			if(IsVoteInProgress()) {
				PrintToChatAll("\x05[花]\x01 投票进行中");
...
public int voteCallback(Menu menu, MenuAction action, int param1, int param2) {
	if(action == MenuAction_VoteEnd) {
		...
		if(param1 == 1 || votes != totalVotes) {
			PrintToChatAll("\x05[花]\x01 投票未全票通过,修改失败");
			return 0;
		}
...
void SetMax(int max) {
	...
	CheckSlots();
	PrintToChatAll("\x05[花]\x01 已修改人数上限为%d人", max);
}
...
public Action Cmd_Kill(int client, int args) {
	if(client && isPlayer(client)) {
		if(IsPlayerAlive(client)) {
			...
			PrintToChatAll("\x05[花] \x04%s \x05升天了...", name);
